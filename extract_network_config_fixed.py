#!/usr/bin/env python3
"""
Fixed script to extract network configuration from lssea*log files
...
def parse_sea_section(lines, start_idx):
    ...
    for i in range(current_idx, len(lines)):
        if "SEA :" in lines[i]:
            # Look for the actual SEA name in the line
            sea_line = clean_text(lines[i])
            sea_match = re.search(r'SEA\s*:\s*(\S+)', sea_line)
            if sea_match:
    ...
    while current_idx < len(lines):
        line = clean_text(lines[current_idx])
...
def main():
    ...
    for file_path in sorted(log_files):
        ...
        if config:
            ...
            # Print details for each SEA
            for i, sea in enumerate(config['sea_sections']):
                print(f"    SEA {i+1}: {sea.get('sea_name', 'Unknown')}")
                if sea.get('etherchannel'):
                    print(f"      ETHERCHANNEL: {sea['etherchannel']['adapters']}")
                print(f"      REAL ADAPTERS: {len(sea.get('real_adapters', []))}")
                print(f"      VIRTUAL ADAPTERS: {len(sea.get('virtual_adapters', []))}")
        else:
            print(f"  Failed to extract configuration")
        print()
    
    # Save to JSON file
    output_file = os.path.join(script_dir, "network_config_fixed.json")
    try:
